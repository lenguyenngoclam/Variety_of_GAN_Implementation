...
def generate_and_visualize(generator, test_inputs, test_targets, epoch):
    ...
    for i in range(6):
        ...
        input_img = cv2.copyMakeBorder((test_inputs[i] * 127.5 + 127.5).numpy().astype('uint8'), 
                                        0, 0, padding, padding, cv2.BORDER_CONSTANT)
        target_img = cv2.copyMakeBorder((test_targets[i] * 127.5 + 127.5).numpy().astype('uint8'),
